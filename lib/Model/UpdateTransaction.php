...
class UpdateTransaction implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setPayeeName($payee_name)
    {
        if (!is_null($payee_name) && (mb_strlen($payee_name) > 50)) {
            throw new \InvalidArgumentException('invalid length for $payee_name when calling UpdateTransaction., must be smaller than or equal to 50.');
        }
    ...
    public function setImportId($import_id)
    {
        if (!is_null($import_id) && (mb_strlen($import_id) > 36)) {
            throw new \InvalidArgumentException('invalid length for $import_id when calling UpdateTransaction., must be smaller than or equal to 36.');
        }
